...
import { classifyManualAlignment } from '../../src/shared/sync.js'
...
describe('padding an uncertain export', () => {
   ...
   * an audio cross-check, say. `addSource` re-solves from metadata, so the weak
  ...
  it('cuts tight when the POV was locked onto a detected match', () => {
    useStore.getState().setProject(emptyProject(), null)
    useStore.getState().addSource(pov('A', '2026-08-17T20:00:00Z'))
    useStore.getState().setActiveSource('A')
    makeClip('Clip 1', 600, 720)
    ...
    // The editor lined it up on the peak the detector found.
    const locked = classifyManualAlignment({
      appliedOffsetSeconds: 0.25,
      detected: { offsetSeconds: 0.25, confidence: 0.94 }
    })
    expect(locked.method).toBe('manual')
    useStore.getState().nudgeSync('B', 0.25, locked)
  ...
  /**
   * The counterpart, and the one that matters: an alignment set away from the
   * detected peak is the editor's judgement, not a measurement. Cutting it
   * tight would produce silently wrong cuts in every clip derived from this
   * POV, discoverable only by watching the exports.
   */
  it('pads when the POV was set away from the detected match', () => {
    useStore.getState().setProject(emptyProject(), null)
    useStore.getState().addSource(pov('A', '2026-08-17T20:00:00Z'))
    useStore.getState().setActiveSource('A')
    makeClip('Clip 1', 600, 720)
    ...
    const estimated = classifyManualAlignment({
      appliedOffsetSeconds: 0.75,
      detected: { offsetSeconds: 0.25, confidence: 0.94 }
    })
    expect(estimated.method).toBe('estimated')
    expect(estimated.confidence).toBeLessThan(0.94)
    useStore.getState().nudgeSync('B', 0.75, estimated)

    const project = useStore.getState().project!
    expect(project.sources.find((x) => x.id === 'B')!.syncMapping!.method).toBe('estimated')
  ...
  it('never locks an alignment with nothing measured behind it', () => {
    const byEye = classifyManualAlignment({ appliedOffsetSeconds: 3, detected: null })
    expect(byEye.method).toBe('estimated')
    expect(byEye.warnings[0]).toMatch(/no audio match/i)
  })
