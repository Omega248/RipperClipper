...
describe('addSyncAnchors', () => {
  ...
  it('never overrides a locked mapping', () => {
    load([pov('a', '2026-08-17T20:00:00Z'), pov('b', '2026-08-17T20:00:30Z')])
    useStore
      .getState()
      .nudgeSync(
        'b',
        5,
        classifyManualAlignment({
          appliedOffsetSeconds: 5,
          detected: { offsetSeconds: 5, confidence: 0.92 }
        })
      )
  ...
  /**
   * An estimate is still the editor's decision. Automatic anchors must not
   * quietly move it back — the editor would have no way to notice.
   */
  it('never overrides an estimated mapping either', () => {
    load([pov('a', '2026-08-17T20:00:00Z'), pov('b', '2026-08-17T20:00:30Z')])
    useStore
      .getState()
      .nudgeSync('b', 5, classifyManualAlignment({ appliedOffsetSeconds: 5, detected: null }))
    const estimated = useStore.getState().project!.sources.find((s) => s.id === 'b')!.syncMapping!
    expect(estimated.method).toBe('estimated')

    useStore.getState().addSyncAnchors([anchor('b', 32, Date.parse('2026-08-17T20:05:00Z') / 1000)])

    const after = useStore.getState().project!.sources.find((s) => s.id === 'b')!.syncMapping!
    expect(after.method).toBe('estimated')
    expect(after.offsetSeconds).toBe(estimated.offsetSeconds)
  })
