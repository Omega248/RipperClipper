...
export class ToolInstaller {
      ...
      if (plan.expected) {
        if (digest.toLowerCase() !== plan.expected.sha256.toLowerCase()) {
          ...
          throw new AppError({
            ...
            message: `The file Ripper Clipper received does not match the checksum ${spec.label}'s publisher lists for it, so it was deleted rather than installed. This is usually a broken download — try again. If it keeps happening, install ${spec.label} yourself and point at it in Settings → Setup.`,
            retryable: true,
      ...
      if (plan.kind === 'binary') {
        ...
        if (installed.length === 0) {
          throw new AppError({
            ...
            message: `The archive downloaded from the publisher did not contain ${(plan.keep ?? []).join(', ')}. Nothing was installed. Install ${spec.label} yourself and point at it in Settings → Setup.`,
            retryable: true
          })
        }
      }
  ...
  private async record(entry: InstalledRecord): Promise<void> {
    ...
    // Same shared-temp-file race as settings had; two tools finishing their
    // install together is exactly when this one would have bitten.
    await atomicWriteJson(join(this.dir, 'installed.json'), [...current, entry])
  }
