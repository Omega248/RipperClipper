 ...
 *
 * Used where a timestamp is context rather than data — "when did this export
 * fail", "when did you last touch this event". An absolute time would be more
 * precise and less useful: the question is whether it was just now or last
 * week, and a relative answer states that without arithmetic.
 */
export function timeAgo(when: number, now: number = Date.now()): string {
